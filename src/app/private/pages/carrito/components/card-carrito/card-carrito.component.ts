...
export class CardCarritoComponent implements AfterViewInit {
  ...
  @Input() producto!:any
  reducir(){
    this.carritoService.decrementarProducto(this.producto.pivot.carrito_id).subscribe((data)=>{
      this.cantidad--;
      this.notificacion.success("Decrementado producto",'Proceso Exitoso!');
    })
  }
  aumentar(){
    this.carritoService.incrementarProducto(this.producto.pivot.carrito_id).subscribe((data)=>{
      this.cantidad++;
